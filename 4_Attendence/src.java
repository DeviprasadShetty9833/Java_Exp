...
public class MultiThreadedBankSimulator {
    ...
    • Single-file: The example keeps everything in one file for easier compilation and running. For real projects, split classes into separate files and packages.
    • Balances in cents: Use integer cents to avoid floating-point rounding issues.
    • Deadlock avoidance: Transfers acquire locks in accountId order and use tryLock with timeouts to reduce chance of deadlock. If a lock cannot be acquired, the transfer is treated as failed (logged).
    • Logger: Uses a BlockingQueue to decouple transaction producers (workers) from the I/O-bound logger. This avoids slowing down workers on file writes.
    • Graceful shutdown: Workers are waited on (Future.get()), scheduler is shutdown, then logger is asked to stop after the queue is drained.
    • Metrics: Atomic counters track successes and failures.
